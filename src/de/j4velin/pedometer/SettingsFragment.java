...
public class SettingsFragment extends PreferenceFragment implements
	...
	final static float DEFAULT_STEP_SIZE = Locale.getDefault() == Locale.US ? 2.5f
			: 75f;
	final static String DEFAULT_STEP_UNIT = Locale.getDefault() == Locale.US ? "ft"
			: "cm";
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		if (((MainActivity) getActivity()).getGC().isConnected()) {
			account.setSummary(getString(R.string.signed_in,
					((MainActivity) getActivity()).getGC().getCurrentPlayer()
							.getDisplayName()));
		}
		...
		goal.setSummary(getString(R.string.goal_summary,
				prefs.getInt("goal", DEFAULT_GOAL)));
	...
	public boolean onPreferenceClick(final Preference preference) {
		...
		case R.string.step_size:
			builder = new AlertDialog.Builder(getActivity());
			prefs = getPreferenceManager().getSharedPreferences();
			v = getActivity().getLayoutInflater().inflate(R.layout.stepsize,
					null);
			final RadioGroup unit = (RadioGroup) v.findViewById(R.id.unit);
			final EditText value = (EditText) v.findViewById(R.id.value);
			unit.check(prefs.getString("stepsize_unit", DEFAULT_STEP_UNIT)
					.equals("cm") ? R.id.cm : R.id.ft);
		...
		case R.string.account:
			...
			if (((MainActivity) getActivity()).getGC().isConnected()) {
				((TextView) v.findViewById(R.id.signedin))
						.append(((MainActivity) getActivity()).getGC()
				...
				builder.setPositiveButton("Sign out",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								((MainActivity) getActivity()).getGC()
										.signOut();
								preference
										.setSummary(getString(R.string.sign_in));
								dialog.dismiss();
							}
						});
			}
			...
			if (Environment.getExternalStorageState().equals(
				...
				try {
					...
					while ((line = in.readLine()) != null) {
						data = line.split(";");
						try {
							db.insertDay(Long.valueOf(data[0]),
				...
				new AlertDialog.Builder(getActivity())
						.setMessage(
								skips > 0 ? skips
										+ " entries were ignored as they did not contain valid data"
		...
		case R.string.export_title:
			if (Environment.getExternalStorageState().equals(
				...
				Database db = new Database(getActivity());
				db.open();
				Cursor c = db.query(new String[] { "date", "steps" }, null,
				...
				new AlertDialog.Builder(getActivity())
						.setMessage("Data saved in " + f.getAbsolutePath())
						.create().show();
			} else {
				new AlertDialog.Builder(getActivity())
						.setMessage("Error: External storage not available")
