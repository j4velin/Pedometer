	...
	static void achievementsAndLeaderboard(final GoogleApiClient gc, final Context context) {
		if (gc.isConnected()) {
			...
			if (!prefs.getBoolean("achievement_boot_are_made_for_walking4", false)) {
				Cursor c = db.query(new String[] { "steps" }, "steps >= 20000", null, null, null, null, "1");
				if (c.getCount() >= 1) {
					unlockAchievement(gc, context.getString(R.string.achievement_boots_made_for_walking_iv));
			...
			if (!prefs.getBoolean("achievement_boot_are_made_for_walking5", false)) {
				Cursor c = db.query(new String[] { "steps" }, "steps >= 25000", null, null, null, null, "1");
				if (c.getCount() >= 1) {
					unlockAchievement(gc, context.getString(R.string.achievement_boots_made_for_walking_v));
			...
			if (days >= 10) {
				...
				if (!prefs.getBoolean("achievement_continual3", false)) {
					if (average > 12500) {
						unlockAchievement(gc, context.getString(R.string.achievement_continual_iii));
